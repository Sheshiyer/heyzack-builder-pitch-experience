...
const CategoryDrawer: React.FC<CategoryDrawerProps> = ({ lang, category, isOpen, onClose, initialProductId }) => {
  ...
  // Sync with initialProductId when drawer opens or initialProductId changes
  React.useEffect(() => {
    if (isOpen && initialProductId) {
      setSelectedProductId(initialProductId);
    } else if (isOpen && !selectedProductId) {
              ...
              <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                {products.map(p => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl mb-2 transition-all group ${selectedProductId === p.id
                  ...
                  >
                    ...
                    <div className={`text-[10px] uppercase font-black tracking-wider mt-1 ${selectedProductId === p.id ? 'text-white/60' : 'text-slate-400'
                      }`}>
                      {p.sku}
                    </div>
                  </button>
